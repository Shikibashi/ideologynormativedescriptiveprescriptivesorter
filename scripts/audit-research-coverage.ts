...
const targetRows = targets.map((target) => {
  ...
  return {
    ...
    profileRouteVariantCount: profile?.routeVariants.length ?? 0,
    profileRouteVariantIds: profile?.routeVariants.map((route) => route.id) ?? [],
...
const report = {
  ...
  researchBank: {
    ...
    targetRowsWithoutProfiles: targetRows.filter((target) => !target.hasAnchorProfile).map((target) => target.id),
    targetRowsWithConceptions: targetRows.filter((target) => target.profileConceptionCount > 0).length,
    targetRowsWithoutConceptions: targetRows.filter((target) => target.profileConceptionCount === 0).map((target) => target.id),
    profileConceptions: targetRows.reduce((total, target) => total + target.profileConceptionCount, 0),
    targetRowsWithRouteVariants: targetRows.filter((target) => target.profileRouteVariantCount > 0).length,
    profileRouteVariants: targetRows.reduce((total, target) => total + target.profileRouteVariantCount, 0),
