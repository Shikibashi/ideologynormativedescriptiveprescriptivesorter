...
test("can complete all layers and create a versioned share link", async ({ page, context }) => {
  ...
  await page.locator('label[for="conception-liberty-institution-pilot-prevent-direct-interference"]').click();
  await page.locator('label[for="political-economy-justice-pilot-real-capabilities"]').click();
  await page.locator('label[for="change-mechanism-pilot-organized-coalition-learning"]').click();
  await page.locator('label[for="change-transition-standard-pilot-affected-can-contest-and-revise"]').click();
  await page.locator('label[for="priority-rights-local-autonomy-pilot-minimum-floor-local-discretion"]').click();
  await page.locator('label[for="epistemic-fact-value-distinction-pilot-separate-then-address"]').click();
  await page.locator('label[for="heterodoxy-revision-pilot-revision-can-preserve-continuity"]').click();
  await page.locator('label[for="priority-liberty-equality-pilot-protect-liberty-first"]').click();
  await page.locator('label[for="priority-ecology-growth-pilot-ecological-limits-first"]').click();
  await page.locator('label[for="priority-scope-members-outsiders-pilot-greatest-need-or-risk"]').click();
  await page.locator('label[for="priority-reform-deep-change-pilot-safeguard-path-to-change"]').click();
  await page.locator('label[for="epistemic-confidence-pilot-tentative-confidence"]').click();
  await page.locator('label[for="epistemic-revision-pilot-revise-the-claim"]').click();
  await page.locator('label[for="epistemic-uncertainty-pilot-withhold-firm-judgment"]').click();
  await page.locator('label[for="epistemic-evidence-change-pilot-new-data-observation"]').click();
  await page.locator('label[for="heterodoxy-dissent-pilot-protect-good-faith-dissent"]').click();
  await page.locator('label[for="heterodoxy-internal-disagreement-pilot-report-meaningful-variation"]').click();
  await page.locator('label[for="heterodoxy-opposition-pilot-protect-organized-opposition"]').click();
  await page.locator('label[for="heterodoxy-minority-response-pilot-retain-open-dissent"]').click();
  ...
  await expect(page.locator(".belief-direct-evidence")).toContainText(/Prevent direct interference/i);
  await expect(page.locator(".belief-direct-evidence")).toContainText(/Real capabilities/i);
  await expect(page.locator(".belief-direct-evidence")).toContainText(/Organized coalition and learning/i);
  await expect(page.locator(".belief-direct-evidence")).toContainText(/Keep the arrangement contestable/i);
  await expect(page.locator(".belief-direct-evidence")).toContainText(/Rights floor with local discretion/i);
  await expect(page.locator(".belief-direct-evidence")).toContainText(/Separate, then address each/i);
  await expect(page.locator(".belief-direct-evidence")).toContainText(/Revision can preserve continuity/i);
  await expect(page.locator(".belief-direct-evidence")).toContainText(/Protect individual freedom first/i);
  await expect(page.locator(".belief-direct-evidence")).toContainText(/Respect ecological limits first/i);
  await expect(page.locator(".belief-direct-evidence")).toContainText(/Prioritize greatest need or risk/i);
  await expect(page.locator(".belief-direct-evidence")).toContainText(/Condition support on safeguards/i);
  await expect(page.locator(".belief-direct-evidence")).toContainText(/Tentative confidence/i);
  await expect(page.locator(".belief-direct-evidence")).toContainText(/Revise the claim/i);
  await expect(page.locator(".belief-direct-evidence")).toContainText(/Withhold a firm judgment/i);
  await expect(page.locator(".belief-direct-evidence")).toContainText(/New data or observation/i);
  await expect(page.locator(".belief-direct-evidence")).toContainText(/Protect good-faith dissent/i);
  await expect(page.locator(".belief-direct-evidence")).toContainText(/Report meaningful variation/i);
  await expect(page.locator(".belief-direct-evidence")).toContainText(/Protect organized opposition/i);
  await expect(page.locator(".belief-direct-evidence")).toContainText(/Retain open dissent/i);
 ...
 await expect(page.locator(".combined-neighbor-list")).toBeVisible();
  ...
  await expect(restored.locator("#conception-liberty-institution-pilot-prevent-direct-interference")).toBeChecked();
  await expect(restored.locator("#political-economy-justice-pilot-real-capabilities")).toBeChecked();
  await expect(restored.locator("#change-mechanism-pilot-organized-coalition-learning")).toBeChecked();
  await expect(restored.locator("#change-transition-standard-pilot-affected-can-contest-and-revise")).toBeChecked();
  await expect(restored.locator("#priority-rights-local-autonomy-pilot-minimum-floor-local-discretion")).toBeChecked();
  await expect(restored.locator("#epistemic-fact-value-distinction-pilot-separate-then-address")).toBeChecked();
  await expect(restored.locator("#heterodoxy-revision-pilot-revision-can-preserve-continuity")).toBeChecked();
  await expect(restored.locator("#priority-liberty-equality-pilot-protect-liberty-first")).toBeChecked();
  await expect(restored.locator("#priority-ecology-growth-pilot-ecological-limits-first")).toBeChecked();
  await expect(restored.locator("#priority-scope-members-outsiders-pilot-greatest-need-or-risk")).toBeChecked();
  await expect(restored.locator("#priority-reform-deep-change-pilot-safeguard-path-to-change")).toBeChecked();
  await expect(restored.locator("#epistemic-confidence-pilot-tentative-confidence")).toBeChecked();
  await expect(restored.locator("#epistemic-revision-pilot-revise-the-claim")).toBeChecked();
  await expect(restored.locator("#epistemic-uncertainty-pilot-withhold-firm-judgment")).toBeChecked();
  await expect(restored.locator("#epistemic-evidence-change-pilot-new-data-observation")).toBeChecked();
  await expect(restored.locator("#heterodoxy-dissent-pilot-protect-good-faith-dissent")).toBeChecked();
  await expect(restored.locator("#heterodoxy-internal-disagreement-pilot-report-meaningful-variation")).toBeChecked();
  await expect(restored.locator("#heterodoxy-opposition-pilot-protect-organized-opposition")).toBeChecked();
  await expect(restored.locator("#heterodoxy-minority-response-pilot-retain-open-dissent")).toBeChecked();
