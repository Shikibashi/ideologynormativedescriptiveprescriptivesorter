...
describe("dataset contract", () => {
  it("accepts the versioned dataset", () => {
    ...
    expect(DATASET.manifest.questionsPerLayer).toEqual({ descriptive: 468, normative: 468, prescriptive: 468 });
...
describe("layer scoring", () => {
  it("keeps no-view separate and fails closed below the coverage threshold", () => {
    ...
    expect(result.layers.descriptive).toMatchObject({ kind: "insufficient-information", answered: 0, total: 468, coverage: 0 });
    expect(result.layers.normative).toMatchObject({ kind: "insufficient-information", answered: 0, total: 468 });
  ...
  it("counts mixed responses as answered while preserving the mixed count", () => {
    ...
    expect(result.layers.descriptive).toMatchObject({ kind: "covered", answered: minimum, total: 468, coverage: 0.5, mixed: minimum });
  });
  ...
  it("keeps contextual bridge anchors inspectable without including them in production scoring", () => {
    expect(DATASET.anchors).toHaveLength(116);
    expect(scoringAnchorsFor(DATASET)).toHaveLength(111);
