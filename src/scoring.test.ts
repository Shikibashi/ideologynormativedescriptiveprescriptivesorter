...
describe("dataset contract", () => {
  it("accepts the versioned dataset", () => {
    ...
    expect(DATASET.manifest.questionsPerLayer).toEqual({ descriptive: 492, normative: 492, prescriptive: 492 });
    ...
    expect(canonicalNodes.filter((node) => node.level === "micro")).toHaveLength(70);
...
describe("layer scoring", () => {
  it("keeps no-view separate and fails closed below the coverage threshold", () => {
    ...
    expect(result.layers.descriptive).toMatchObject({ kind: "insufficient-information", answered: 0, total: 492, coverage: 0 });
    expect(result.layers.normative).toMatchObject({ kind: "insufficient-information", answered: 0, total: 492 });
  ...
  it("counts mixed responses as answered while preserving the mixed count", () => {
    ...
    expect(result.layers.descriptive).toMatchObject({ kind: "covered", answered: minimum, total: 492, coverage: 0.5, mixed: minimum });
  });
  ...
  it("keeps contextual bridge anchors inspectable without including them in production scoring", () => {
    expect(DATASET.anchors).toHaveLength(122);
    expect(scoringAnchorsFor(DATASET)).toHaveLength(117);
