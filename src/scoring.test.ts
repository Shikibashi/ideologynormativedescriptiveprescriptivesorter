...
describe("dataset contract", () => {
  it("accepts the versioned dataset", () => {
    ...
    expect(DATASET.manifest.questionsPerLayer).toEqual({ descriptive: 484, normative: 484, prescriptive: 484 });
    ...
    expect(canonicalNodes.filter((node) => node.level === "meso")).toHaveLength(37);
...
describe("layer scoring", () => {
  it("keeps no-view separate and fails closed below the coverage threshold", () => {
    ...
    expect(result.layers.descriptive).toMatchObject({ kind: "insufficient-information", answered: 0, total: 484, coverage: 0 });
    expect(result.layers.normative).toMatchObject({ kind: "insufficient-information", answered: 0, total: 484 });
  ...
  it("counts mixed responses as answered while preserving the mixed count", () => {
    ...
    expect(result.layers.descriptive).toMatchObject({ kind: "covered", answered: minimum, total: 484, coverage: 0.5, mixed: minimum });
  });
  ...
  it("keeps contextual bridge anchors inspectable without including them in production scoring", () => {
    expect(DATASET.anchors).toHaveLength(120);
    expect(scoringAnchorsFor(DATASET)).toHaveLength(115);
