...
## V125 continuation — resolution posture and defining support

The current implementation advances the same separation boundary to `configuration-projection` version 5. Candidate records now expose defining-commitment support as a count and proportion, so total observed configuration coverage cannot be mistaken for coverage of the commitments that the source-backed configuration treats as defining.

The result also carries an explicit resolution record. It remains `insufficient-information` when the three-layer threshold is unmet, `not-derived` when no configuration has enough defining support, `coarse-neighborhood` when the leading candidates have low internal-fit separation, and `provisional-neighborhood` otherwise. The record carries at most the five leading candidate ids for inspection and never selects a unique ideology label. A coarse result directs future authors toward source-backed neighbor-discriminating research items before any scorer change.

This is an interpretive and measurement observability change only. The candidate order, fit calculation, existing separation policy, production questions, anchor vectors, legacy scorer, and external validation ledger remain unchanged. The new fields are structural diagnostics; they are not calibrated confidence, respondent classification, cognitive evidence, psychometric evidence, invariance evidence, population evidence, or empirical validity.

## V149 continuation — layered morphology without forced synthesis

The existing ontology already distinguishes macro families, meso traditions, micro currents, and typed hybrid relations. The morphology result previously retained the ontology level on each candidate but did not expose that shape in the result card, and its aggregate construct fit did not show how descriptive, normative, and prescriptive commitments contributed to the observed evidence. That made a mixed profile harder to inspect without implying that one ordered label was the whole account.

The current candidate contract therefore adds `interpretationKind`, `hybridOfIds`, and a `layerSupport` record for all three claim layers. `layerSupport` repeats the existing commitment-level agreement calculation as a trace, but restricts construct evidence to observations from the same claim layer: it reports directional agreement only where same-layer directional construct evidence exists, and separately reports the proportion of directional commitment weight observed plus distinct observed/total commitment counts. It is not a new affinity score, a layer weight, a threshold, a calibrated confidence measure, or a selection rule. Candidate order and the no-unique-label resolution state remain unchanged.

The UI also reports counts of these ontology shapes across the full provisional grid and shows the layer trace on each visible candidate. The implementation and canonical morphology audit require all 119 source-backed configurations to round-trip with complete layer slots and ontology-consistent hybrid metadata. This is structural observability evidence only; it does not establish that respondents understood the items, that the layers are psychometrically separable, or that any morphology label is valid for a person or population.
