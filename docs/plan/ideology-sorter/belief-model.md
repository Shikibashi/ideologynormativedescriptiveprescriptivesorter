...
The executable `npm run belief:morphology-audit` also round-trips all 119 current canonical configurations through finite synthetic answer maps directed by each configuration's source-backed commitments. All 119 canonical configurations are source-backed, with zero anchor-only fallbacks; each has commitments, an explicit conceptual representation, and the five explicit relational-constraint gaps. Every target configuration is present in its candidate set, while rank is retained as a coarse overlap diagnostic rather than a coefficient-tuning objective. An all-mixed profile now returns `not-derived` with an empty candidate set, so under-determined configuration records cannot leak through the public morphology contract. The same audit exercises hybrid, weak, and descriptive-only profiles; same-values/different-causal-belief profiles; same-policy/different-principle profiles; alternative priority and conditional rules; unresolved contradiction visibility; direct and relational affinity isolation; and branch-metadata isolation. These fixtures are structural and adversarial diagnostics; they are not respondent, cognitive, psychometric, invariance, population, or empirical validation.
