...
The audit is non-destructive. A `rewrite`, `split`, or `remap` disposition is a review queue, not a silent content edit. The current production questions are all `facet-proxy` observations. The optional direct pilot is a separate, non-production evidence surface; its records do not turn the three production-unmeasured constructs into production measurements. The default measurement-audit command emits the full item-level records; its `--summary` mode emits only aggregate counts.
...
The results page offers thirty optional direct-belief pilot items in `src/belief-direct-items.ts`. They ask the respondent to select the closest account of a concept, moral scope, causal mechanism, legitimacy basis, distributive reason, institutional route, political economy, change mechanism, change path, transition standard, priority rule, epistemic stance, or response to internal contestation. Nineteen of the records carry explicit lineage to the nineteen quarantined research candidates, so candidate coverage is inspectable without promoting the candidate shelf. A selection becomes `BeliefDirectEvidence` with the chosen option text, construct links, layer, and the option’s substantive source references; it is not converted into a numeric observation. Item-level `sourceRefs` remain the question-design basis, so selected evidence does not inherit a broader source bundle merely because those sources supported the prompt. The pilot therefore makes a reason, conception, mechanism, standard, or relationship stance visible without silently changing the legacy scorer or claiming that categorical options constitute a latent scale.

The pilot uses a separate answer map, preserves “No view yet” as missing, validates item and option ids plus option-level source links before profile or share use, checks that each linked research candidate matches the direct item's construct, layer, and source set, and serializes selected pairs in the versioned share envelope. Its current output is explicitly provisional and is excluded from morphology calculation pending expert adjudication, response-process evidence, cross-context checks, and empirical validation. This is an implementation seam for the next research loop, not a claim that the thirty prompts have passed those gates.
...
The executable `npm run belief:measurement-audit` report for the current content version 99 records 1,500 production items, evenly distributed as 500 descriptive, 500 normative, and 500 prescriptive items. All 1,500 are facet proxies and zero are direct construct items; the separate direct pilot has thirty items and does not change that production count. The V131 question-layer repair removed six exact duplicate-wording findings through five source-backed prompt rewrites, and the V132 repair removed the remaining single-item rewrite signal through one additional source-backed prompt rewrite; all existing effect maps remain unchanged. V146 added one optional item for each production-unmeasured construct, V147 added seven optional items for the remaining candidate-only claim-layer cells, and V148 adds twelve candidate-specific records for the remaining priority, epistemic, and heterodoxy candidates while preserving those constructs and cells as unmeasured in the production audit. The direct-pilot and completion audits now require exactly one linked direct record for every quarantined research candidate. The disposition counts and mechanical flags are regenerated by the command because source-backed tranche additions and wording repairs can change them. The current uncovered constructs remain `priority-conflict`, `epistemic-stance`, and `heterodoxy-contestation`, while the ten candidate-only claim-layer cells remain explicit research gaps.
...
At content version 99, the 42 `split` signals comprise 10 descriptive, 11 normative, and 21 prescriptive items. Twenty-one bridge two constructs, ten bridge three, nine bridge four, and two bridge five; 13 also carry a condition/contrast flag while 29 do not. This distribution is intentionally not treated as a global rewrite rule: some records are compact relational or institutional claims whose coordinated wording may still express one intended unit, while others visibly combine distinct diagnosis, value, institutional, distributive, or change claims. The queue therefore requires item-level content adjudication, and the aggregate `split` disposition does not by itself establish that any item is invalid or must be removed.
...
## V149 — layered morphology trace

The primary morphology candidate now makes two existing distinctions explicit for result consumers. `interpretationKind` reports whether the canonical configuration is a macro family, meso tradition, micro current, or an existing typed hybrid formation; `hybridOfIds` carries only the configuration's direct `hybrid-of` ontology relations. `layerSupport` decomposes the same construct-level provisional agreement and commitment coverage into descriptive, normative, and prescriptive slots, using construct signals restricted to observations from the corresponding claim layer. It does not introduce a fourth layer, a second scorer, a new weight, a threshold, or a new label.

The per-layer fields are deliberately asymmetric: directional agreement is omitted when that layer has no observed directional commitment evidence, while commitment coverage remains zero or partial. This prevents missing descriptive, normative, or prescriptive evidence from being displayed as a neutral middle position. The overall fit, candidate ordering, nearest-candidate margin, abstention posture, legacy compatibility path, and source-backed configuration set remain unchanged. The result view also reports the inventory of ontology shapes in the full candidate grid so a hybrid does not disappear merely because the top-five inspectable neighborhood is coarse.

The change is an interpretability and measurement-trace improvement grounded in the existing morphology boundary: ideological configurations combine concepts across layers, but a proximity ordering is not an essence or a validated classification. Local structural and browser checks verify that every canonical configuration round-trips with all three layer slots and that the metadata matches the ontology. These checks do not establish respondent comprehension, expert agreement, reliability, validity, invariance, population/consequence safety, or held-out morphology performance; the six external gates remain `NOT RUN`.

### Route-aware target-question trace

The canonical question-coverage report now carries a separate `routeVariantCoverage` record for every source-backed-contested prescriptive route variant. The ten routes attached to Populism, Islamism, Religious Nationalism, and Deep Ecology are joined to their existing target-tagged four-item prescriptive blocks by target id. Each record preserves route statement/source ids and exposes the route's determinate facet ids, covered/uncovered facets, item-level alignments, and structural status.

This trace is deliberately weaker than a universal family direction: the four base profiles remain `contested-indeterminate`, while a route can pass because its alternative has a mapped target block. A missing route mapping is a structural failure and does not get repaired by manufacturing a family commitment. The route trace remains non-scoring qualitative context and does not enter production questions, response observations, morphology fit, legacy scoring, affinity, or promotion. The summary and completion audit require all discovered route variants to have this trace; this is a local content contract only, not respondent, cognitive, psychometric, reliability, validity, invariance, population, or consequence evidence.
