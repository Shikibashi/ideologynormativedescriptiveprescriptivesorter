...
## V147 direct-pilot candidate-only layer extension

| ID | Scenario | Expected result |
|---|---|---|
| P-147-01 | Complete the production quiz and inspect the optional direct-belief pilot. | The existing result surface renders 18 categorical records, including the seven new records for prescriptive concept-conception, normative economic justice, descriptive change mechanism, normative transition standard, prescriptive rights/local-autonomy priority, normative fact/value distinction, and normative heterodoxy revision; each has four substantive options and explicit `No view yet` missingness. |
| R-147-01 | Select one option for each of the seven new records, create a share link, and restore it. | All seven selected option texts are visible in the direct-evidence profile and all seven selected pairs restore from the versioned share payload; the profile remains non-scoring. |
| R-147-02 | Run the direct-pilot and completion audits. | The direct audit reports 18 items, ten candidate-only construct/layer cells, `directPilotCoversCandidateOnlyCells: true`, zero production overlap, and unchanged legacy/affinity traces. The completion audit reports a structurally eligible but overall `INCOMPLETE` result because all six external gates remain `NOT RUN`. |
| R-147-03 | Run TypeScript, focused/full Vitest, the production build, research/measurement/morphology/coverage audits, fresh packet validation, and browser QA against a stable preview. | All executed local checks pass; the production bank remains 1,500 `facet-proxy` questions, the packet expands to 1,543 records with promotion false, no cognitive review or respondent study is performed, and all six external gates remain `NOT RUN`. |

## V148 candidate-to-direct-pilot lineage

| ID | Scenario | Expected result |
|---|---|---|
| P-148-01 | Complete the production quiz and inspect the optional direct-belief pilot. | The existing result surface renders 30 categorical records, including four additional priority-rule records, four descriptive epistemic-stance records, and four heterodoxy/contestation records; every record has four substantive options and explicit `No view yet` missingness. |
| R-148-01 | Select one option for each of the twelve V148 records, create a share link, and restore it. | All twelve selected option texts are visible in the non-scoring direct-evidence profile and all twelve selected pairs restore from the versioned share payload; the production score and morphology affinity remain unchanged. |
| R-148-02 | Run the direct-pilot and completion audits. | The direct audit reports 30 items, 19 candidate-specific direct items, one direct item for every quarantined candidate, all ten candidate-only cells covered, zero production overlap, and zero validation errors. The completion audit reports the same lineage check as structurally passing while remaining `INCOMPLETE` because the six external gates are `NOT RUN`. |
| R-148-03 | Run TypeScript, focused/full Vitest, production build, measurement/morphology/research/question-coverage audits, fresh packet export/validation, and browser QA against a stable preview. | All executed local checks pass; the production bank remains 1,500 `facet-proxy` items, the review queue is 1,555 records, promotion remains false, and no cognitive review, respondent study, or external validation result is created. |

## V149 layered morphology trace

| ID | Scenario | Expected result |
|---|---|---|
| P-149-01 | Complete the production quiz and inspect the primary morphology candidates. | The candidate grid reports the existing macro-family, meso-tradition, micro-current, and hybrid-formation shapes where present; each visible candidate shows descriptive, normative, and prescriptive directional agreement/commitment coverage, and any direct `hybrid-of` relations. The copy says this is a non-selecting interpretive trace. |
| R-149-01 | Run the morphology audit against all canonical configurations. | All 119 source-backed configurations round-trip with finite bounded layer-support coverage, valid observed/total commitment counts, and interpretation metadata matching ontology level and direct hybrid relations. Candidate ordering, fit, margins, and resolution remain unchanged. |
| R-149-02 | Run TypeScript, focused/full Vitest, production build, morphology/completion audits, fresh packet export/validation, and stable-preview browser QA. | Executed local checks pass; the production bank remains 1,500 `facet-proxy` questions, the review packet remains structurally incomplete with promotion false, and all six external validation gates remain `NOT RUN`. No cognitive review or respondent study is performed. |

## V150 route-aware target-question trace

| ID | Scenario | Expected result |
|---|---|---|
| R-150-01 | Run `npm run belief:question-coverage -- --summary` with the current source snapshot. | The report audits 119 canonical targets, reports ten route variants across Populism (2), Islamism (2), Religious Nationalism (2), and Deep Ecology (4), marks every route trace `pass`, and keeps the four base-family prescriptive open gaps unchanged. |
| R-150-02 | Inspect a route row in the full question-coverage JSON. | Each route retains its id, statement, source ids, prescriptive layer, four existing target question ids, determinate/covered facet ids, item alignments, and non-scoring structural status; no route becomes a production question or selector. |
| R-150-03 | Run the broken-effect regression fixture that removes all Populism prescriptive target effects. | Both Populism route traces become `gap`, the route structural check and failure list fail closed, and Populism's base prescriptive posture remains `contested-indeterminate` with its existing open gap. |
| R-150-04 | Run the full deterministic audit surface, fresh packet validation, and stable-preview browser suite after V150. | Executed local checks pass; the production bank and review queue counts remain unchanged, route metadata remains non-scoring, packet promotion remains false, and the six external validation gates remain `NOT RUN`. No cognitive review or respondent study is performed. |
